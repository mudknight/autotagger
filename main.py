...
def download_model_files(repo_id):
    """Downloads the ONNX model and the tags CSV from a specified Hugging Face repository."""
    print(f"Fetching model artifacts from Hugging Face repository: {repo_id}...")
...
def main():
    parser = argparse.ArgumentParser(description="Auto-tag images using SmilingWolf tagger models via OpenVINO.")
    parser.add_argument("directory", type=str, help="Directory containing images to tag.")
    parser.add_argument("-m", "--model", type=str, default="SmilingWolf/wd-v1-4-moat-tagger-v2", 
                        help="Hugging Face repository ID for the target model (default: SmilingWolf/wd-v1-4-moat-tagger-v2).")
    parser.add_argument("-d", "--dry-run", action="store_true", help="Print tags to stdout instead of saving to file.")
    parser.add_argument("-c", "--confidence", type=float, default=0.37, help="Tag confidence threshold (default: 0.37).")
    parser.add_argument("-g", "--gpu", type=str, default="GPU", help="Target hardware accelerator device index (default: GPU).")
    parser.add_argument("-u", "--keep-underscores", action="store_true", help="Keep underscores in tags instead of replacing them with spaces.")
    ...
    print(f"Detected model input resolution requirement: {image_size}x{image_size}")
    
    # Scan target directory for images
    files = [os.path.join(args.directory, f) for f in os.listdir(args.directory)]
    image_paths = [f for f in files if os.path.isfile(f) and f.lower().endswith(VALID_EXTENSIONS)]
    ...
    for image_path in image_paths:
        try:
            ...
            if args.dry_run:
                print(f"--- Dry Run Results for: {os.path.basename(image_path)} ---")
                print(tag_string if tag_string else "[No tags matched threshold]")
                print("-" * 40 + "\n")
            else:
                ...
                print(f"Successfully processed: {os.path.basename(image_path)} -> {os.path.basename(txt_path)}")
                
        except Exception as e:
            print(f"Failed to process {os.path.basename(image_path)}. Error: {e}")
